...
bool agent_constructor_getters_test()
{
	...
	Agent agent(student, works, age, xi, yi, hID, is_hospital_patient, sID, wID, works_at_hospital, hspID, infected);
	agent.set_ID(aID);
...
bool agent_out_test()
{
	bool student = false, works = true, infected = false;
	bool is_hospital_patient = false, works_at_hospital = true;
	int school_type = 3;
	int age = 30, hID = 50, sID = 0, wID = 10001, hspID = 1;
	...
	res >> test_aID >> test_student >> test_works >> test_age
		...
		>> test_works_hospital >> test_hspID >> test_infected;
