	...
	for (int ti = 0; ti<=tmax; ++ti){
		for (auto& agent : agents){
			...
			if (agent.infected() == false){
				hsp_em.susceptible_transitions(agent, time, infection,
				...
				// No exposed - hospital staff under IH
	...
	for (int ti = 0; ti<=tmax; ++ti){
		for (auto& agent : agents){
			...
			if (agent.infected() == false){
				...
				hsp_em.exposed_transitions(agent, infection, time, dt, 
					households, schools, hospitals, 
	...
	for (int ti = 0; ti<=tmax; ++ti){
		for (auto& agent : agents){
			...
			if (agent.infected() == false){
				...
				hsp_em.symptomatic_transitions(agent, time, dt, infection,  
