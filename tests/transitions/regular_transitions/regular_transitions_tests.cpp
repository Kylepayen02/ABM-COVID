...
bool test_exposed_transitions()
{
	...
	int n_ih = 0, n_tc = 0, n_hsp = 0, n_false_neg = 0;
	...
	for (const auto& agent : agents){
		...
		if (agent.exposed()){
			...
			if (agent.tested_exposed() || agent.tested_false_negative()){
				...
				++n_tested_exposed;
				++n_tested;
				if (!check_tests(agent, time_to_res, n_tc, n_hsp, n_false_neg, n_ih, 
	...
	// Fractions should match to first decimal
	if (!check_fractions(n_tested_sy, n_sy, 
			infection_parameters.at("fraction to get tested"), "fraction to get tested"))
		return false;
	if (!check_fractions(n_hsp, n_tested, 
			infection_parameters.at("fraction tested in hospitals"), "fraction tested in hospitals"))
		return false;

	// This is because some may be awating 
	// results so already tested
	if (n_tested < (n_tc + n_hsp))
		return false;
	// Home isolated
	if (n_ih > n_tested)
		return false;
	// False negative	
	if (static_cast<double>(n_false_neg) > static_cast<double>(n_tested)
...
bool test_symptomatic_transitions()
{
			...
			if (agent.infected() == false){
				...
				if (agent.being_treated()){
					if (agent.hospitalized() || agent.hospitalized_ICU()){
						// Home should be removed and hospital added
						int hID = agent.get_hospital_ID();
						if ( hID > hospitals.size() || hID <= 0)
							return false;
						if (!find_in_place<Hospital>(hospitals, agent.get_ID(), hID))
						...
						if (agent.hospitalized()){
							if (agent.recovering()){
								t_hsp_to_ih.push_back(agent.get_time_hsp_to_ih());
							} else {
								t_hsp_to_icu.push_back(agent.get_time_hsp_to_icu());
							}
							++n_hsp;
						}
						if (agent.hospitalized_ICU()){
							++n_icu;
							if (agent.recovering()){
								t_icu_to_hsp.push_back(agent.get_time_icu_to_hsp());
							} 
						}
					} else {
						...
						if (!check_isolation(agent, schools, workplaces))
							return false;
						// And house ID for home isolated
						if (!find_in_place<Household>(households, agent.get_ID(), agent.get_household_ID()))
							return false;
						if (!agent.home_isolated())
							return false;
						if (agent.recovering()){
							t_ih_to_hsp.push_back(agent.get_time_ih_to_hsp());
						} else {
							t_ih_to_icu.push_back(agent.get_time_ih_to_icu());
						}
						++n_ih;
					}
				}
	...
	// All three treatments should exist
	if (n_ih == 0 || n_hsp == 0 || n_icu == 0)
		return false;
	// Same for tested false negative
	if (n_false_neg == 0)
		return false;
	// And all removal 
