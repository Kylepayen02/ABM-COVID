				...
				std::vector<Agent>& agents)
{
	...
	if (infection.infected(lambda_tot) == true){
		...
		set_testing_status(agent, infection, time, hospitals, infection_parameters);
	} 
	return got_infected;	
}
...
int HspPatientTransitions::exposed_transitions(Agent& agent, Infection& infection, const double time, const double dt, 
...
void HspPatientTransitions::testing_results_transitions(Agent& agent, 
