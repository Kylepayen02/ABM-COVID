...
public:
	 ...
	 */
	void add_exposed(double inf_var) override { lambda_sum += inf_var*beta_employee; }

	/** 
	 *  \brief Include exposed contribution in the sum
	 *	@param inf_var - agent infectiousness variability factor
	 */
	void add_exposed_patient(double inf_var) { lambda_sum += inf_var*beta_non_covid_patient; }

	/** 
	 *  \brief Include symptomatic  contribution in the sum
	 *	@param inf_var - agent infectiousness variability factor
	 */
	void add_symptomatic_patient(double inf_var) { lambda_sum += inf_var*ck*beta_non_covid_patient; }

	/** 
	 *  \brief Include tested at hospital contribution in the sum
	 *	@param inf_var - agent infectiousness variability factor
	 */
	void add_hospital_tested(double inf_var) { lambda_sum += inf_var*ck*beta_testee; }

	/** 
	 *  \brief Include tested at hospital contribution in the sum
	 *	@param inf_var - agent infectiousness variability factor
	 */
	void add_exposed_hospital_tested(double inf_var) { lambda_sum += inf_var*beta_testee; }

	/** 
	 *  \brief Include hospitalized contribution in the sum
	 *	@param inf_var - agent infectiousness variability factor
	 */
	void add_hospitalized(double inf_var) { lambda_sum += inf_var*ck*beta_hospitalized; }

	/** 
	 *  \brief Include hospitalized in ICU contribution in the sum
	 *	@param inf_var - agent infectiousness variability factor
	 */
	void add_hospitalized_ICU(double inf_var) { lambda_sum += inf_var*ck*beta_hospitalized_ICU; }
    ...
    void reset_contributions() override 
		{ lambda_tot = 0.0; lambda_sum = 0.0; num_infected = 0.0; n_tested = 0; }
